# Queller Bot Runner — sources (version 53, 14 Sep 2026)
...
Tests (run all before building):
    ...
    node test/verify.js   # scripted scenarios for every issue fixed in version 52
...
ref.js (glossary, rules, rulings, turn sequence), engine.js (state, dice, cards, playability, table triggers; `VERSION`), walk.js (flowchart walker), debug.js (action history, error capture, the exportable debug log — kept outside the game state, persisted under `qb.debug`), ui.js, modals.js, styles.css.

build.py stamps `window.QB_BUILT` (UTC build time) into the page; bump `VERSION` in engine.js for each published build.
