...
(async () => {
  ...
  await answerAll();
  let s = await S();
  console.log("strategy:", s.strategy, "phase:", s.phase);
  ...
  console.log(
    ...
    s.dice.hunt,
    "avail",
    s.dice.pool.filter((d) => d.st === "avail").length,
    "phase",
    s.phase,
  );
  // 2. Phase 5 walks until Phase 6 appears (bounded)
  for (let i = 0; i < 20; i++) {
    const p5 = await page.$('[data-phase="p5"]');
    if (!p5) break;
    await p5.click();
    await answerAll();
  }
  ...
  console.log(
    ...
    s.dice.pool.filter((d) => d.st === "avail").length,
    "reserved",
    s.dice.pool.filter((d) => d.st === "reserved").length,
    "log",
    s.log.length,
  );
  ...
  await page.evaluate(() => {
    window.QBUI.act(() => {
      ...
      S.cards.discards.C = S.cards.discards.C.filter(
        (i) => !["sa051", "sa045", "sa009"].includes(i),
      );
    });
  });
  ...
  const dlg = await page.$("#modal");
  console.log(
    "ask dialog open:",
    !!dlg,
    dlg ? await page.textContent("#mtitle") : "",
  );
  if (dlg) await page.click('#modal [data-ask="ok"]');
  s = await S();
  console.log("after answer: table", JSON.stringify(s.cards.table));
  await page.check("#t-fs-revealed");
  const dlg2 = await page.$("#modal");
  console.log("Flocks ask:", dlg2 ? await page.textContent("#mtitle") : "none");
  if (dlg2) await page.click('#modal [data-ask="no"]');
  // 5. table card details + reminder, die tap, undo, minimal tracker fields via settings
  const det = await page.$('[data-card="show"]');
  if (det) {
    await det.click();
    console.log(
      "details shown:",
  ...
  s = await S();
  console.log("rings now", s.board.rings);
  ...
  if (s.turn !== turnBefore)
    dbgErrors.push("failed action was not rolled back");
  await page.evaluate(() =>
    setTimeout(() => {
  ...
  if (!log.summary.some((t) => /2 errors recorded/.test(t)))
    dbgErrors.push("summary lacks the error count");
  await click("#mclose");
  ...
  console.log(
    ...
    log.actions.length,
    "page loads",
    log.actions.filter((a) => a.a === "pageLoad").length,
  );
  if (
    log.actions.length < actionsBefore ||
    log.actions.filter((a) => a.a === "pageLoad").length < 2
  )
    dbgErrors.push("action history did not survive the reload");
  await click("#mclose");
  ...
  const unexpected = errors.filter(
    (e) => !/smoke: deliberate|could not render the saved game/.test(e),
  );
  console.log(
    unexpected.length
      ? "ERRORS:\n" + unexpected.join("\n")
