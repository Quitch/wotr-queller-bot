...
- `npm run coverage` writes `coverage/lcov.info`; the CI `verify` job hands it to SonarQube Cloud (CI-based analysis, `sonar-project.properties`, the `SONAR_TOKEN` repository secret), so Sonar's coverage figure is the Node-side coverage only: the browser smoke test runs in the separate `browser` job and is not in it. The modules that only run in a browser (`main.js`, `ui/boot.js`, `ui/wire.js`, `ui/tooltip.js`, `modals/framework.js`) are excluded from Sonar's coverage measure (`sonar.coverage.exclusions`) for that reason; keep DOM wiring in those modules and pure logic where the Node tests reach it.
